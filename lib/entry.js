  ...
  walker.on('files', function (root, stats, callback) {
    ...
    if (hasContent) {
      ...
      prepare.prepare(opts, function (err, results) {
        ...
        contentIDMap[root] = results.contentIDBase;
        callback(null);
      });
    } else {
      callback(null);
...
exports.preparePullRequest = function (opts, callback) {
  ...
  var getPresentedURLMap = function (cb) {
    ...
    async.forEachOf(contentIDMap, function (contentRoot, contentID, cb) {
      var whereis = urlJoin('_api', 'whereis', encodeURIComponent(contentID));
      presenterAPI(whereis, function (err, resp, body) {
        if (err) return cb(err);

        if (resp.statusCode !== 200) {
          opts.say('Unsuccessful response returned from staging presenter API.');
