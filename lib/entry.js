...
exports.preparePullRequest = function (opts, callback) {
  ...
  var commentOnGitHub = function (cb) {
    ...
    var environmentRootURL = urlJoin(opts.stagingPresenterURL, revisionID);

    if (!githubAPI) {
      opts.say('Unable to comment on GitHub: no GitHub account available.');
      opts.say('Your preview is available at %s', environmentRootURL);
      return cb(null);
    }
    ...
    githubAPI.post({
      url: u,
      body: { body: comment.forSuccessfulBuild({'/': environmentRootURL}) }
    }, function (err, resp) {
      if (err) return cb(err);
