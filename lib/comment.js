...
var mappingHelp = 'Your build succeeded, but your content is not mapped anywhere ' +
...
exports.forSuccessfulBuild = function (urlMap) {
  ...
  for (var contentRoot in urlMap) {
    ...
    if (stagedURLs.length === 0) {
      ...
      for (var i = 0; i < stagedURLs.length; i++) {
        if (i !== 0) {
          message += ', ';
        }
        message += util.format('[%s](%s)', stagedURLs[i], stagedURLs[i]);
      }
    }
  }
