...
var readConfiguration = function (state) {
  return function (callback) {
    ...
    fs.readFile(filename, { encoding: 'utf-8' }, function (err, contents) {
      ...
      try {
        ...
        state.say('Unable to parse _deconst.json file in %s', state.filename);
        return callback(new Error('Unable to parse _deconst.json'));
      }

      if (state.revisionID) {
        if (config.contentIDBase) {
          state.contentIDBase = config.contentIDBase;

          // Prepend the revision ID as the first path segment of the content ID.
          var u = url.parse(config.contentIDBase);

          var parts = u.pathname.split('/');
          while (parts[0] === '') {
            parts.shift();
          }
          parts.unshift(state.revisionID);
          u.pathname = '/' + parts.join('/');

          state.contentIDBase = url.format(u);
          state.whisper('Revised content ID base: [%s]', state.contentIDBase);
        } else {
          state.say('No content ID base found in %s', state.filename);
...
var createPreparerContainer = function (state) {
  return function (callback) {
    ...
    if (state.contentIDBase) {
      env.push("CONTENT_ID_BASE=" + state.contentIDBase);
    }
    ...
    if (state.dataContainer) {
      ...
      params.Env.push("CONTENT_ROOT=" + state.root);
    } else {
...
exports.prepare = function (opts, callback) {
  ...
  async.series([
    ...
    var result = { contentIDBase: state.contentIDBase };
